...
class REST
{
    ...
    private function scopeReservation()
    {
        ...
        if (!$reservation) {
            ...
            $this->response['reason'] = 'Not a valid reservation ID';
        ...
        switch ($this->params['operation']) {
            ...
            case 'ics':
                ...
                if (Functions\getSettings()->getShowIcal()) {
                    Files\generateICSFile(
                        Toolkit\filterInput($reservation->getServiceName(), TRUE) . '.ics',
                        $reservation->getStart(),
                        $reservation->getEnd(),
                        $reservation->getServiceLocation(),
            ...
            case 'cancel':
                ...
                if ($reservation->isCancelled()) {
                    $this->integrateWithTheme(
                        ...
                        . '</h2>',
                        sprintf(esc_html__('Reservation %s', 'team-booking'), '#' . $reservation->getDatabaseId())
                    );
                    $this->response['output'] = TRUE;
                    break;
                }
                if ($this->service->getSettingsFor('customer_cancellation') && $this->service->getClass() !== 'unscheduled') {
                    ...
                    if ($now + $this->service->getSettingsFor('cancellation_allowed_until') < $reservation->getStart()) {
                        if ($this->params['show_confirm']) {
                            ...
                            $this->integrateWithTheme(
                                '<h2>'
                                . esc_html__('Reservation succesfully cancelled!', 'team-booking')
                                . '</h2>',
                                sprintf(esc_html__('Reservation %s', 'team-booking'), '#' . $reservation->getDatabaseId())
                            );
                            $this->response['output'] = TRUE;
            ...
            case 'approve':
                ...
                if ($this->params['show_confirm']) {
                    ...
                    $this->integrateWithTheme(
                        '<h2>' . esc_html__('Reservation succesfully approved!', 'team-booking') . '</h2>',
                        sprintf(esc_html__('Reservation %s', 'team-booking'), '#' . $reservation->getDatabaseId())
                    );
                    $this->response['output'] = TRUE;
                }
            ...
            case 'decline':
                ...
                if ($this->params['show_confirm']) {
                    ...
                    $this->integrateWithTheme(
                        '<h2>' . esc_html__('Reservation succesfully declined!', 'team-booking') . '</h2>',
                        sprintf(esc_html__('Reservation %s', 'team-booking'), '#' . $reservation->getDatabaseId())
                    );
                    $this->response['output'] = TRUE;
                }
    ...
    {
        $when_value = \DateTime::createFromFormat('U', $reservation->getStart());
        $when_value->setTimezone(new \DateTimeZone($reservation->getCustomerTimezone()));
        $service = Database\Services::get($reservation->getServiceId());
        ob_start();
        ?>
        <h2 class="entry-title"><?= esc_html($question_text) ?></h2>
        <p>
            <strong><?= $reservation->getServiceName() ?></strong>
            <?php printf(esc_html__('on %1$s at %2$s', 'team-booking'),
                '<strong>' . Functions\date_i18n_tb(get_option('date_format'), $when_value->getTimestamp() + $when_value->getOffset()) . '</strong>',
                '<strong>' . Functions\date_i18n_tb(get_option('time_format'), $when_value->getTimestamp() + $when_value->getOffset()) . '</strong>'
            ); ?>
            (<?= str_replace('_', ' ', $when_value->getTimezone()->getName()) ?>)
        </p>
    ...
    <?php } ?>
        ...
        <?php
        $this->integrateWithTheme(ob_get_clean(), sprintf(esc_html__('Reservation %s', 'team-booking'), '#' . $reservation->getDatabaseId()));
        exit;
    }
    ...
    private function showPayment(\TeamBooking_ReservationData $reservation)
    {
        ...
        if ($reservation->isPaid()) {
            ...
            <div>
                <table>
                    ...
                    <tr>
                        ...
                        <td>
                            <?= Functions\currencyCodeToSymbol($reservation->getCurrencyCode(), $reservation->getPriceIncremented()) ?>
                        </td>
                    </tr>
                    </tbody>
                </table>
            ...
            <script>
                jQuery(document).ready(function () {
                    jQuery('.wrapper').on('click keydown', '.tbk-pay-button', function (e) {
                        if (e.which == 13 || e.which == 32 || e.which == 1) {
     ...
     */
    private function integrateWithTheme($content, $page_title = '')
    {
        ...
        if (locate_template('page.php') !== '') {
            ...
            add_filter('the_content', function ($old_content) use ($content) {
                return Functions\api_page_main_content('<div class="tbk-api-wrapper">' . $content . '</div>');
            });
            get_template_part('page');
        } else {
            $title = '<h1>' . $page_title . '</h1>';
            get_header();
            Functions\api_page_main_content('<div class="tbk-api-wrapper">' . $title . $content . '</div>');
            get_footer();
        }
    }
