...
class Admin
{
    public function __construct()
    {
        ...
        add_action('tb-db-cleaning-routine', 'TeamBooking\\Functions\\tbCleanReservationsRoutine');
    ...
    public function addAdminMenu()
    {
        ...
        if (current_user_can('manage_options')) {
            ...
            $menu_label = 'TeamBooking '
                . "<span class='update-plugins tb-bubble count-$bubble_count' title='" . esc_attr($bubble_title) . "'><span class='update-count'>"
                . number_format_i18n($bubble_count)
                . '</span></span>';

            $page_hook = add_menu_page('Team Booking', $menu_label, 'manage_options', 'team-booking', array(
                $this,
                'createAdminPage',
            ...
            $page_hook = add_menu_page('Team Booking', $menu_label, 'tb_can_sync_calendar', 'team-booking', array(
                $this,
                'createCoworkerPage',
            ), TEAMBOOKING_URL . 'images/admin-icon.png');
            add_action("load-$page_hook", array(
    ...
    {
        if (!isset($_GET['tab']) || $_GET['tab'] === 'overview') {
            $args = array(
                'label'   => esc_html__('Reservations per page', 'team-booking'),
                'default' => 20,
                'option'  => 'tbk_reservations_per_page'
            );
            add_screen_option('per_page', $args);
        } elseif ($_GET['tab'] === 'slots') {
            $args = array(
    ...
    public function enqueueResources($hook)
    {
        ...
        if ($hook === 'toplevel_page_team-booking') {
            ...
            wp_enqueue_script('tb-admin-script', TEAMBOOKING_URL . 'js/admin.js', array(
                ...
                'jquery-ui-core',
                'jquery-ui-sortable',
            ), filemtime(TEAMBOOKING_PATH . 'js/admin.js'));
    ...
    public function checkForNotices()
    {
        ...
        if (isset($_GET['nag_duplicated_linked_title'])) {
            Notices\personal_duplicated_event_title();
        }
        ...
        <div class="wrap">
            ...
            if (isset($_GET['whatsnew'])) {
                ...
                if (isset($_GET['tab'])) {
                    ...
                    Admin\Misc::render()->getTabWrapper($_GET['tab']);
        ...
        <div class="wrap">
            ...
            if (isset($_GET['tab'])) {
                ...
                if (in_array($_GET['tab'], $allowed_tabs)) {
                    // Select the active tab, if present...
                    Admin\Misc::render()->getTabWrapperCoworker($_GET['tab']);
                    $function_to_call = 'buildAdminTab' . $_GET['tab'];
                    $this->$function_to_call();
                } else {
    ...
    {
        if (!current_user_can('manage_options')) wp_die(__('You are not allowed to be on this page.', 'team-booking'));
        check_admin_referer('team_booking_options_verify');
        ...
        foreach ($_POST as $setting => $value) {
            ...
            if ($setting === 'debug') Functions\getSettings()->setSilentDebug(filter_input(INPUT_POST, 'debug') === 'silent');
        ...
        if (isset($_FILES['settings_json_file']['tmp_name'])) {
            ...
            if ($settings instanceof \stdClass) {
                if ($settings->web->client_id
                    ...
                    Functions\getSettings()->save();
                    $response = admin_url('admin.php?page=team-booking&tab=general&nag_settings_imported=1');
                } else {
                    // Invalid file
    ...
    private function saveServiceSettings()
    {
        ...
        if ($_POST['service_settings'] === 'general' && current_user_can('manage_options')) {
            foreach ($_POST['event'] as $setting => $value) {
                ...
                if ($setting === 'allow_reservation') $service->setSettingsFor('logged_only', filter_var($value, FILTER_VALIDATE_BOOLEAN));
    ...
    public function confirmPendingPaymentReservation()
    {
        ...
        if ($updated_record instanceof \TeamBooking_ReservationData) {
            ...
            $location = admin_url('admin.php?page=team-booking&tab=overview&nag_reservation_confirmed=1');
        } elseif ($updated_record instanceof \TeamBooking_Error) {
            /**
             * Something goes wrong
             */
    ...
    public function cancelServiceReservation()
    {
        ...
        if (!$reservation_db_record) {
            return add_query_arg('nag_generic_error', 1, $url);
        }
        if (!current_user_can('manage_options')) {
            try {
                if ($reservation_db_record->getCoworker() != get_current_user_id()
                    || Database\Services::get($reservation_db_record->getServiceId())->getSettingsFor('approval_rule') !== 'coworker'
                ) {
    ...
    private function saveCoworkerServiceSettings($email = FALSE)
    {
        ...
        ///////////////////////////
        if (!$email) {
            $services_id_list = Functions\getSettings()->getServiceIdList();
            ...
            // Checks for duplicate linked event title or not allowed strings
            if ((in_array($linked_event_title_lowercase, $other_linked_event_titles)
                    && Database\Services::get($_POST['service_id'])->getClass() !== 'unscheduled')
                ...
                if (isset($_POST['data']['booked_title'])) {
                    if (strpos('||', $_POST['data']['booked_title']) !== FALSE) {
                        exit(wp_redirect(add_query_arg('nag_duplicated_linked_title', 1, $location)));
                    }
                ...
                if (isset($_POST['data']['default_duration'])) {
                    $settings->setFixedDuration($_POST['data']['default_duration']['hours'] * HOUR_IN_SECONDS + $_POST['data']['default_duration']['minutes'] * MINUTE_IN_SECONDS);
                }
                if (isset($_POST['data']['buffer'])) {
                    $settings->setBufferDuration($_POST['data']['buffer']['hours'] * HOUR_IN_SECONDS + $_POST['data']['buffer']['minutes'] * MINUTE_IN_SECONDS);
                }
    ...
    public function saveServiceCustomField()
    {
        try {
            ...
            if (array_key_exists($parsed_data[ $_POST['hook'] . '_hook' ], $fields)
                ...
                \TeamBooking\Functions\backend_form_field_save($parsed_data, $service, $_POST['hook']);
    ...
    public function insertPromotionCampaign()
    {
        ...
        $start_date = \DateTime::createFromFormat('m/d/Y', $_POST['campaign_start']);
        $end_date = \DateTime::createFromFormat('m/d/Y', $_POST['campaign_end']);
        ...
        $errors = '';
        if (!$start_date || $start_date->format('m/d/Y') != $_POST['campaign_start']) {
            $errors .= 'start_date_fail';
        }
        if (!$end_date || $end_date->format('m/d/Y') != $_POST['campaign_end']) {
    ...
    public function insertPromotionCoupon()
    {
        ...
        $start_date = \DateTime::createFromFormat('m/d/Y', $_POST['coupon_start']);
        $end_date = \DateTime::createFromFormat('m/d/Y', $_POST['coupon_end']);
        $bound_start_date = \DateTime::createFromFormat('m/d/Y', $_POST['promotion_bound_start']);
        $bound_end_date = \DateTime::createFromFormat('m/d/Y', $_POST['promotion_bound_end']);
        $errors = '';
        if (!$start_date || $start_date->format('m/d/Y') != $_POST['coupon_start']) {
            $errors .= 'start_date_fail';
        }
        if (!$end_date || $end_date->format('m/d/Y') != $_POST['coupon_end']) {
            $errors .= 'end_date_fail';
        }
        if (count(Database\Promotions::getByName($name)) > 0) {
            $errors .= 'name_fail';
    ...
    public function editPromotion()
    {
        ...
        $start_date = \DateTime::createFromFormat('m/d/Y', $_POST['pricing_start']);
        $end_date = \DateTime::createFromFormat('m/d/Y', $_POST['pricing_end']);
        $bound_start_date = \DateTime::createFromFormat('m/d/Y', $_POST['promotion_bound_start']);
        $bound_end_date = \DateTime::createFromFormat('m/d/Y', $_POST['promotion_bound_end']);
        $errors = '';
        if (!$start_date || $start_date->format('m/d/Y') != $_POST['pricing_start']) {
            $errors .= 'start_date_fail';
        }
        if (!$end_date || $end_date->format('m/d/Y') != $_POST['pricing_end']) {
            $errors .= 'end_date_fail';
        }
