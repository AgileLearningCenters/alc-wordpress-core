...
class Parser
{
    ...
    public function setRequestedMinTime($time = NULL)
    {
         ...
         */
        if (strtotime($time) < current_time('timestamp')) {
            $now = new \DateTime();
            $now->setTimezone($this->timezone);
            $time = $now->format(\DateTime::RFC3339);
        }
    ...
    {
        $min_time = \DateTime::createFromFormat(\DateTime::RFC3339, $this->min_time_requested);
            ...
            foreach ($events as $calendar_id => $items) {
                foreach ($items as $event_id => $event) {
                    ...
                    if (isset($this->service_matches[ $coworker_id ][ static::parseSummary($event->summary) ])) {
                        ...
                        if ($custom_time_bounds->reference === 'start') {
                            $time = \DateTime::createFromFormat($slot->isAllDay() ? 'Y-m-d' : \DateTime::ISO8601, $slot->getStartTime());
                        } else {
                            $time = \DateTime::createFromFormat($slot->isAllDay() ? 'Y-m-d' : \DateTime::ISO8601, $slot->getEndTime());
                        }
                        ...
                        if ($match['status'] === 'free' && in_array($match['service'], $this->services_requested)) {
                        ...
                        if (substr(static::parseSummary($event->summary), -10) === ' container') {
                            ...
                            $slot->setReadOnly($event->creator !== static::$settings->getCoworkerData($coworker_id)->getAuthAccount());
    ...
    private function splitContainers()
    {
        ...
        foreach ($this->slots_containers as $slot) {
            ...
            if (!$duration) {
                 ...
                 */
                $start_time_spool = $start_time_obj->format(DATE_RFC3339);
                $end_time_spool = $end_time_obj->format(DATE_RFC3339);
                ...
                foreach ($contained_booked_slots as $booked_slot) {
                    if (strtotime($booked_slot->getStartTime()) >= strtotime($start_time_spool)) {
                        ...
                        if ($service_id !== $booked_slot->getServiceId()
                            ...
                            while ((strtotime($booked_slot->getEndTime()) - strtotime($start_time_spool)) >= ($buffer + $duration)) {
                                $slot_to_add = clone $slot;
                                $slot_to_add->setEventIdParent($slot->getEventId());
                                $slot_to_add->setEventId(FALSE);
                                $slot_to_add->setContainer(FALSE);
                ...
                // residual
                if (strtotime($start_time_spool) < strtotime($end_time_spool)) {
                    while ((strtotime($end_time_spool) - strtotime($start_time_spool)) >= $duration) {
                        $slot_to_add = clone $slot;
                        $slot_to_add->setEventIdParent($slot->getEventId());
                        $slot_to_add->setEventId(FALSE);
                        $slot_to_add->setContainer(FALSE);
    ...
    protected function getCoworkerTimeBoundaries($coworker_id, $service_id, $min_get_time)
    {
         ...
         */
        if (strtotime($min_get_time) < strtotime($now->format(\DateTime::RFC3339))) {
            /**
             ...
             */
            $min_time = $now->format(\DateTime::RFC3339);
        } else {
            /**
        ...
        if ($open_time_string) {
            ...
            $open_time = $now->format(\DateTime::RFC3339);
        }
    ...
    {
        $overlap = FALSE;
        $date_obj = new \DateTime($check->getStartTime(), $this->timezone);
        $xs = $date_obj->format('U');
        $date_obj = new \DateTime($check->getEndTime(), $this->timezone);
        $xe = $date_obj->format('U');
        $date_obj = new \DateTime($main->getStartTime(), $this->timezone);
        $ds = $date_obj->format('U');
        $date_obj = new \DateTime($main->getEndTime(), $this->timezone);
        $de = $date_obj->format('U');
