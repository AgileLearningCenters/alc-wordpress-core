...
class ReservationsTable extends \WP_List_Table
{
    ...
    public function get_views()
    {
        ...
        $status_links = array(
            'all' => '<a href="'
                . admin_url('/admin.php?page=team-booking&tab=overview') . '"'
        ...
        $status_links['upcoming'] = '<a href="'
            . admin_url('/admin.php?page=team-booking&tab=overview&filter=upcoming') . '"'
        ...
        if ($this->total_items_unscheduled > 0) {
            $status_links['unscheduled'] = '<a href="'
                . admin_url('/admin.php?page=team-booking&tab=overview&filter=unscheduled') . '"'
        ...
        if ($this->total_items_pending > 0) {
            $status_links['pending'] = '<a href="'
                . admin_url('/admin.php?page=team-booking&tab=overview&filter=pending') . '"'
        ...
        if ($this->total_items_waiting_approval > 0) {
            $status_links['waiting_approval'] = '<a href="'
                . admin_url('/admin.php?page=team-booking&tab=overview&filter=waiting_approval') . '"'
    ...
    protected function get_items()
    {
        ...
        if (isset($_POST['s']) && !empty($_POST['s'])) {
            if (current_user_can('manage_options')) {
                $this->reservations = Reservations::getBySearch(filter_input(INPUT_POST, 's', FILTER_SANITIZE_STRING), 0, $this->current_page, $this->order_by, $this->order);
            } else {
                $this->reservations = Reservations::getByCoworkerSearch(get_current_user_id(), filter_input(INPUT_POST, 's', FILTER_SANITIZE_STRING), 0, $this->current_page, $this->order_by, $this->order);
            }
            $this->set_pagination_args(array(
        ...
        foreach ($this->reservations as $reservation) {
            ...
            $date_time_of_reservation_value = Framework\Html::span(array(
                'text'  =>
                    Functions\date_i18n_tb(get_option('date_format'), $date_time_object->getTimestamp() + $date_time_object->getOffset())
            ...
            if (NULL !== $reservation->getStart()) {
                ...
                if ($reservation->isAllDay()) {
                    ...
                    $when_value = Functions\date_i18n_tb(get_option('date_format'), $date_time_object->getTimestamp() + $date_time_object->getOffset())
                        . Framework\Html::span(array(
                            'text'  => Functions\date_i18n_tb(get_option('time_format'), $date_time_object->getTimestamp() + $date_time_object->getOffset())
                                . ' - '
            ...
            $items[ $reservation->getDatabaseId() ] = array(
                ...
                'service'        => Functions\checkServiceIdExistance($reservation->getServiceId())
                    ? (Framework\Html::anchor(array(
                            'href'  => get_admin_url() . 'admin.php?page=team-booking&tab=events&event=' . $reservation->getServiceId(),
                            'text'  => $reservation->getServiceName(),
                            'class' => 'tbk-to-sort'
                        )) . Framework\Html::span(array(
                            'text'  => Services::get($reservation->getServiceId())->getClass(TRUE),
                            'class' => 'tbk-description'
                        )))
                    : ($reservation->getServiceName() . Framework\Html::span(array(
                ...
                ,
                'when'           => $when_value,
                'date'           => '<strong>#' . $reservation->getDatabaseId()
            ...
            if ($reservation->isPending() && !$reservation->isPaid()) {
                ...
                $items[ $reservation->getDatabaseId() ]['payment_status'] .= Framework\Html::span(array(
                    'text'  => Functions\currencyCodeToSymbol($reservation->getCurrencyCode(), $reservation->getPriceIncremented() * $reservation->getTickets()),
    ...
    public function getReservationActions(\TeamBooking_ReservationData $reservation)
    {
        ...
        if (($pending || $reservation->isPending()) && current_user_can('manage_options')) {
