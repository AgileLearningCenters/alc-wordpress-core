...
class SlotsTable extends \WP_List_Table
{
    ...
    public function __construct()
    {
        ...
        $max_get_time = $now->format(\DateTime::RFC3339);
        ...
        if (current_user_can('manage_options')) {
            ...
            $fetch->setRequestedCoworkers(array(get_current_user_id()));
            $fetch->sync();
    ...
    public function extra_tablenav($which = 'top')
    {
        ...
        $interval_url = remove_query_arg('interval', $current_url);
        ...
        if ($which === 'top') {
            ...
            foreach (Services::get() as $service) {
                ...
                echo '<option value="' . $option_url . '" ' . selected($filter_service, $service->getId()) . '>' . $service->getName() . '</option>';
            }
            echo '</select>';
    ...
    protected function get_items()
    {
        ...
        $items = array();
        foreach ($this->slots as $slot_id => $slot) {
            $date_time_object = new \DateTime($slot->getStartTime(), $timezone);
            $date_time_object->setTimezone($timezone);
            $date_time_object_end = new \DateTime($slot->getEndTime(), $timezone);
            $date_time_object_end->setTimezone($timezone);
            if ($slot->isAllDay()) {
                $slot_times = Functions\date_i18n_tb(get_option('date_format'), $date_time_object->getTimestamp())
                ...
                $slot_times = '<strong>' . Functions\date_i18n_tb(get_option('date_format'), $date_time_object->getTimestamp() + $date_time_object->getOffset()) . '</strong>'
                    . Framework\Html::span(array(
            ...
            $items[ $slot_id ] = array(
                ...
                'service'       => Functions\checkServiceIdExistance($slot->getServiceId())
                    ? (Framework\Html::anchor(array(
                            'href'  => get_admin_url() . 'admin.php?page=team-booking&tab=events&event=' . $slot->getServiceId(),
                            'text'  => $slot->getServiceName(),
                            'class' => 'tbk-to-sort'
                        )) . Framework\Html::span(array(
    ...
    public function getSlotActions(Slot $slot)
    {
        ...
        $modal->setHeaderText(array(
            'main' => $slot->getServiceName() . ', ' . $slot->getDateString(),
        ...
        $modal->additionalButton(array(
            ...
            'data'  => array(
                ...
                'filename'     => sanitize_file_name($slot->getServiceName() . '_' . $slot->getDateString() . '_' . $slot->getTimesString()) . '.csv'
            )
        ));
        $modal->additionalButton(array(
            ...
            'data'  => array(
                'reservations' => implode(',', $reservation_ids),
                'filename'     => sanitize_file_name($slot->getServiceName() . '_' . $slot->getDateString() . '_' . $slot->getTimesString()) . '.xlsx'
            )
        ));
        $modal->render();
