...
class Service
{
    ...
    public function getPostBodyEmail()
    {
        ...
        <div class="tbk-wrapper">
            ...
            <form method="POST" action="<?= $this->action ?>">
                ...
                if (current_user_can('manage_options')) {
                    $column = Framework\Column::ofWidth(4);
                    $column->addElement($this->getEmailCancellationSettings($hooks));
                    $column->appendTo($row);
                }
                $row->render();
                ?>
    ...
    public function getPostBodyGcalSettings()
    {
        ...
        <div class="tbk-wrapper">
            ...
            $column->addElement(Framework\ElementFrom::content($this->getHeaderBlock(
                ...
                __('Those settings are applied to your Google Account only', 'team-booking') . '<br>' . Framework\Html::span(array('text' => $this->coworker->getAuthAccount(), 'class' => 'active'))
            )));
            $column->appendTo($row);
            $row->render();
    ...
    /**
     * The admin e-mail and confirmation e-mail part
     *
     * @param array $hooks
     *
    ...
    {
        $string_18 = esc_html__("E-mail reminders are sent via WordPress Cron system, which is triggered by site's visits. If your site has low traffic, then reminders can have delays, or not be sent at all.", 'team-booking');
        $panel = new Framework\Panel(ucfirst(__('Admin notification and customer confirmation', 'team-booking')));

        // Admin address for receiving notifications
        $element = new Framework\PanelSettingText(__('Admin address for receiving notifications', 'team-booking'));
        $element->addDescription(__('This address will be the sender of confirmation e-mail to customers', 'team-booking'));
        ...
        $element = new Framework\PanelSettingEmailEditor(__('Notification e-mail message to admin', 'team-booking'));
        ...
        $element = new Framework\PanelSettingEmailEditor(__('Confirmation e-mail message to customer', 'team-booking'));
        $element->setShowSend(TRUE);
        ...
        if ($this->service->getClass() !== 'unscheduled') {
            $element = new Framework\PanelSettingEmailEditor(__('Reminder e-mail message to customer', 'team-booking'));
            $element->setShowSend(TRUE);
            ...
            $element->addDescription(Framework\Html::paragraph($string_18), FALSE);
            ob_start();
            ?>
    ...
    private function getEmailCoworkerSettings(array $hooks)
    {
        ...
        $element = new Framework\PanelSettingEmailEditor(__('Notification e-mail message', 'team-booking'));
    ...
    /**
     * The cancellation e-mail part
     *
     * @param array $hooks
     *
     * @return Framework\Panel
     */
    private function getEmailCancellationSettings(array $hooks)
    {
        $panel = new Framework\Panel(ucfirst(__('Cancellation notification', 'team-booking')));

        // Send an e-mail message back to customer when a reservation is cancelled
        $element = new Framework\PanelSettingEmailEditor(__('Cancellation e-mail message to customer', 'team-booking'));
        $element->setShowSend(TRUE);
        ...
        $element = new Framework\PanelSettingEmailEditor(__('Cancellation e-mail message to admin and coworker', 'team-booking'));
        $element->setShowSend(TRUE);
        ...
        <div class="tbk-panel tbk-content">
            <div class="tbk-settings-title">
                <?= Framework\Html::anchor(array(
                    ...
                    'href'   => admin_url('admin.php?page=team-booking&tab=events')
                )) ?>
                <div style="display: inline-block;">
                    <?= Framework\Html::h3(array('text' => $this->service->getName(), 'class' => 'tbk-heading')) ?>
    ...
    private function getGeneralEdit()
    {
        ...
        if ($this->service->getClass() === 'event') {
            ...
            $element->addAlert(__('The limit will be ignored by logged-in Administrators', 'team-booking'));
        ...
        if ($this->service->getClass() !== 'unscheduled') {
            ...
            $element->addOption(array(
                ...
                'label_title' => __('Let Coworkers decide', 'team-booking'),
                'value'       => 'coworker',
        ...
        if ($this->service->getClass() === 'unscheduled') {
            ...
            $element->addOption(array(
                ...
                'label_title' => __('Equal', 'team-booking'),
                'value'       => 'equal',
            ...
            $element->addOption(array(
                ...
                'label_title' => __('Direct', 'team-booking'),
                'value'       => 'direct',
            ...
            $element->addOption(array(
                ...
                'label_title' => __('Random', 'team-booking'),
                'value'       => 'random',
        ...
        $element->addDescription(__("If a location is set, a map will appear on the reservation form. If inherited from Google Calendar slot, but the slot has no location, the map won't show.", 'team-booking'));
        $element->addFieldname('event[location_setting]');
        $element->addOption(array(
            'label'       => __('No location', 'team-booking'),
            'label_title' => __('No location', 'team-booking'),
            'value'       => 'none',
            'checked'     => (int)('none' === $this->service->getSettingsFor('location'))
        ));
        if ($this->service->getClass() !== 'unscheduled') {
            $element->addOption(array(
                'label'       => __('Inherited from Google Calendar slot', 'team-booking'),
                'label_title' => __('Inherited from Google Calendar slot', 'team-booking'),
                'value'       => 'inherited',
        ...
        $element->addOption(array(
            ...
            'label_title' => __('Fixed location', 'team-booking'),
            'value'       => 'fixed',
    ...
    private function getGeneralPayments()
    {
        ...
        $element->setFieldDescription(Functions\currencyCodeToSymbol(Functions\getSettings()->getCurrencyCode(), NULL));
        $panel->addElement($element);
        ...
        $element->addOption(array(
            ...
            'label_title' => __('Immediately', 'team-booking'),
            'value'       => 'immediately',
        ...
        $element->addOption(array(
            ...
            'label_title' => __('Later', 'team-booking'),
            'value'       => 'later',
        ...
        $element->addOption(array(
            ...
            'label_title' => __("At the customer's discretion", 'team-booking'),
            'value'       => 'discretional',
    ...
    private function getGeneralRedirect()
    {
        ...
        $element->addOption(array(
            ...
            'label_title' => __('No redirect', 'team-booking'),
            'value'       => 'no',
        ...
        $element->addOption(array(
            ...
            'label_title' => __('Redirect to URL', 'team-booking'),
            'value'       => 'yes',
            'checked'     => $this->service->getSettingsFor('redirect'),
            'description' => $description_element
        ));
    ...
    private function getGeneralSlotsAppearance()
    {
        ...
        if ($this->service->getClass() === 'event') {
            ...
            $element->addOption(array(
                ...
                'label_title' => __('Yes', 'team-booking'),
                'value'       => 1,
            ...
            $element->addOption(array(
                ...
                'label_title' => __('No', 'team-booking'),
                'value'       => 0,
            ...
            $element->addOption(array(
                ...
                'label_title' => __('Yes under this threshold', 'team-booking'),
        ...
        if ($this->service->getClass() !== 'unscheduled') {
            ...
            $element->addOption(array(
                ...
                'label_title' => __('Show start/end', 'team-booking'),
                'value'       => 'yes',
            ...
            $element->addOption(array(
                ...
                'label_title' => __('Show start time only', 'team-booking'),
    ...
    private function getGeneralApproveDeny()
    {
        ...
        $element->addOption(array(
            ...
            'label_title' => __('Do not require approval', 'team-booking'),
            'value'       => 'none',
        ...
        if ($this->service->getClass() !== 'unscheduled') {
            ...
            $element->addOption(array(
                ...
                'label_title' => __('keep the slot/tickets free (could lead to overbooking)', 'team-booking'),
                'value'       => 'yes',
                'checked'     => $this->service->getSettingsFor('free_until_approval')
            ));
            $element->addOption(array(
                'label'       => __('set the slot/tickets in a booked status', 'team-booking'),
                'label_title' => __('set the slot/tickets in a booked status', 'team-booking'),
                'value'       => 'no',
                'checked'     => !$this->service->getSettingsFor('free_until_approval')
            ));
            $panel->addElement($element);
    ...
    private function getGeneralAccess()
    {
        ...
        $element->addOption(array(
            ...
            'value'       => 0,
            'checked'     => $this->service->getSettingsFor('logged_only') == FALSE
        ));
        $element->addOption(array(
            'label'       => __('Logged users only', 'team-booking'),
            'label_title' => __('Logged users only', 'team-booking'),
            'value'       => 1,
            'checked'     => $this->service->getSettingsFor('logged_only') == TRUE
        ));
        $panel->addElement($element);
