...
class Upcoming
{
    ...
    public static function getView(\TeamBooking\RenderParameters $parameters, $read_only = FALSE)
    {
        ...
        ob_start();
        if (!$parameters->getIsAjaxCall()) echo \TeamBooking\Frontend\Calendar::getCalendarStyle();
        ?>
        <?php if (!$parameters->getIsAjaxCall() && !$parameters->getNoTimezone()) { ?>
        <div class="tbk-main-calendar-settings tbk-noselection">
            <div>
                <?php if (!$parameters->getNoTimezone()) { ?>
                    <div class="tbk-setting-button tbk-timezones" tabindex="0" style="margin: 0"
                        ...
                        <?= Functions\timezone_list($parameters->getTimezone()->getName(), $parameters->getIsWidget()) ?>
                    </div>
                <?php } ?>
            </div>
        </div>
                ...
                <div class="tbk-slide">
                    <ul>
                        ...
                        foreach ($slots as $slot) {
                            ...
                            echo '<div style="display: block"><div class="tbk-service-name">' . $slot->getServiceName()
                                . '</div>'
                                . Schedule::getPriceTag($service, $slot)
                            ...
                            if ($parameters->getShowServiceDescriptions()) {
                                echo '<div class="tbk-service-desc">' . $service->getDescription() . '</div>';
                            }
                            echo '</div>';
                            echo '</li>';
