...
class TeamBooking_PaymentGateways_PayPal_Settings implements TeamBooking_PaymentGateways_Settings
{
     ...
     * @return bool
     */
    public function getDataForm($data, $reservation_database_id)
    {
        return FALSE;
    }
    ...
    public function prepareGateway(TeamBooking_ReservationData $data, $additional_parameter = NULL)
    {
        ...
        $paypal->addItem($data->getServiceName(), $data->getPriceIncremented(), $data->getTickets());
        $paypal->setIpnId($data->getToken());
        // should we override the redirect URL for this service?
        if (Database\Services::get($data->getServiceId())->getSettingsFor('redirect')) {
            $paypal->setRedirectUrl(Database\Services::get($data->getServiceId())->getRedirectUrl($data->getDatabaseId()));
        } else {
    ...
    public function getBackendSettingsTab()
    {
        ...
        $element->addFieldname('gateway_settings[' . $this->gateway_id . '][business_email]');
        ...
        $element->addAlertDropcap(__('Note', 'team-booking'));
        $element->addNotice(__('ensure your primary PayPal e-mail address is this otherwise payment notifications will fail.', 'team-booking'));
    ...
    public function listenerIPN($ipn_data)
    {
        ...
        if ($verified) {
            ...
            if (!$reservation) { // Reservation is not found!
