...
class TeamBooking_PaymentGateways_Stripe_Settings implements TeamBooking_PaymentGateways_Settings
{
    ...
    public function prepareGateway(TeamBooking_ReservationData $data, $additional_parameter = NULL)
    {
        ...
        $stripe->addItem($data->getServiceName() . ' | ' . $data->getCustomerDisplayName(), $data->getPriceIncremented(), $data->getTickets());
    ...
    /**
     * @param string $reservation_checksum
     * @param int    $reservation_database_id
     *
     * @return string
     */
    public function getDataForm($reservation_checksum, $reservation_database_id)
    {
        ob_start();
        $form_id = 'tb-stripe-payment-form-' . mt_rand(100000, 999999);
        $reservation = TeamBooking\Database\Reservations::getById($reservation_database_id);
        $url = 'https://stripe.com';
        ?>
        <?= \TeamBooking\Frontend\Components\NavigationHeader::InPaymentForm(Functions\priceFormat($reservation->getPriceIncremented() * $reservation->getTickets())) ?>
            ...
            jQuery('form[id^="tb-stripe-payment-form-"]')
                ...
                .on('click keydown', '.tbk-submit', function (e) {
                    ...
                    if (e.which == 13 || e.which == 32 || e.which == 1) {
                        ...
                        jQuery.tbkSliderGet($tbk_form.closest('.tb-frontend-calendar')).adaptHeight();
                        // Disable the submit button to prevent repeated clicks
                        $tbk_form.find('.tbk-submit').addClass('disabled tbk-loading');
