...
class Promotions
{
    ...
    {
        $promotions = Promotions::getAll($running);
        $return = array();
     ...
     * @return \TeamBooking\Promotions\Promotion[]
     */
    public static function getByClass($class, $running = FALSE)
    {
        return Promotions::getBy('class', $class, $running);
    }
    ...
    {
        $promotions = Promotions::getByService($service_id, $active, $running);
        foreach ($promotions as $promotion_id => $promotion) {
    ...
    public static function getAll($running = FALSE, $filter = FALSE, $per_page = 0, $page_number = 0, $order_by = 'id', $order = 'asc')
    {
        ...
        if ($order_by === 'status') {
            ...
            $usages = Functions\count_used_discounts(Reservations::getAll());
