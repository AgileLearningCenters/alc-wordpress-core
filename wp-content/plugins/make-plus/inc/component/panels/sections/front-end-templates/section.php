...
class TTFMP_Panels_Frontend_Section {
	 ...
	 */
	public function __construct( $section_data, $sections ) {
		...
		if ( isset( $this->section_data['mode'] ) ) {
			$this->mode = ttfmake_sanitize_section_choice( $this->section_data['mode'], 'mode', 'panels' );
		} else {
		...
		$this->items = $this->section_data['panels-items'];
	}
	...
	private function get_attributes( $section_data, $sections ) {
		...
		$full_width = isset( $data['full-width'] ) && 0 !== absint( $data['full-width'] );
		if ( true === $full_width ) {
			$attributes['class'] .= ' builder-section-full-width';
		}

		// Styles
		$attributes['style'] = '';
		if ( ! empty( $data['background-image'] ) ) {
			$image_id = ttfmake_sanitize_image_id( $data['background-image'] );
			$attributes['style'] .= 'background-image: url(' . addcslashes( $this->get_image_url( $image_id ), '"' ) . ');';
		}
		if ( '' !== $data['background-color'] ) {
			$attributes['style'] .= 'background-color: ' . maybe_hash_hex_color( $data['background-color'] ) . ';';
		}
	...
	public function render() {
		...
		<?php if ( '' !== $this->section_data['title'] ) : ?>
			<h3 class="builder-banner-section-title">
				<?php echo sanitize_text_field( $this->section_data['title'] ); ?>
			</h3>
		<?php endif; ?>
		<?php if ( ! empty( $this->items ) ) :
			$heightStyle = esc_js( ttfmake_sanitize_section_choice( $this->section_data['height-style'], 'height-style', 'panels' ) );
			?>
		...
		<?php endif; ?>
		<?php if ( 0 !== absint( $this->section_data['background-image-darken'] ) ) : ?>
	...
	private function render_accordion( $items ) {
		foreach ( $items as $item ) : ?>
			<div id="ttfmp-panels-item-title-<?php echo esc_attr( $item['id'] ); ?>" class="ttfmp-panels-item-title">
				<?php echo apply_filters( 'title_save_pre', $item['item-title'] ); ?>
			</div>
			<div id="ttfmp-panels-item-content-<?php echo esc_attr( $item['id'] ); ?>" class="ttfmp-panels-item-content">
				<?php ttfmake_get_builder_save()->the_builder_content( $item['item-content'] ); ?>
			</div>
	<?php endforeach;
	}
	...
	private function render_tabs( $items ) {
		...
		<?php foreach ( $items as $item ) : ?>
			<li id="ttfmp-panels-item-title-<?php echo esc_attr( $item['id'] ); ?>" class="ttfmp-panels-item-title">
				<a href="#ttfmp-panels-item-content-<?php echo esc_attr( $item['id'] ); ?>">
					<?php echo apply_filters( 'title_save_pre', $item['item-title'] ); ?>
				</a>
			</li>
		<?php endforeach; ?>
