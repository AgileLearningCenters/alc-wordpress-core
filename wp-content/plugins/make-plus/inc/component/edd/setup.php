...
final class MAKEPLUS_Component_EDD_Setup extends MAKEPLUS_Util_Modules implements MAKEPLUS_Util_HookInterface {
	...
	public function admin_enqueue( $hook_suffix ) {
		...
		if (
            ...
            wp_enqueue_script(
                'makeplus-edd-model',
                makeplus_get_plugin_directory_uri() . 'js/edd/builder-model.js',
                $dependencies,
                MAKEPLUS_VERSION,
                true
            );
            ...
            wp_enqueue_script(
                'makeplus-edd-view',
                makeplus_get_plugin_directory_uri() . 'js/edd/builder-view.js',
                $dependencies,
                MAKEPLUS_VERSION,
                true
            );
	...
	public function section_defaults( $defaults ) {
		$defaults['downloads'] = $this->get_defaults();
		...
		ttfmake_add_section(
			...
			array(
				...
				200 => array(
					...
					'name'    => 'full-width',
					'default' => ttfmake_get_section_default( 'full-width', 'edd-downloads' ),
				),
				300 => array(
					'type'  => 'image',
	...
	public function get_section_json( $data ) {
		if ( $data['section-type'] == 'downloads' ) {
	...
	public function save_downloads( $data ) {
		...
		$defaults = array(
			...
			'full-width'       => ttfmake_get_section_default( 'full-width', 'edd-downloads' ),
