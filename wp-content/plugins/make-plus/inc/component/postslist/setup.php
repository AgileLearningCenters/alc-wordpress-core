...
final class MAKEPLUS_Component_PostsList_Setup extends MAKEPLUS_Util_Modules implements MAKEPLUS_Util_HookInterface {
		...
		ttfmake_add_section(
			...
			array(
				...
				200 => array(
					...
					'name'    => 'full-width',
					'default' => ttfmake_get_section_default( 'full-width', 'postlist' ),
				),
				300 => array(
					'type'  => 'image',
					...
					'default' => ttfmake_get_section_default( 'background-image', 'postlist' ),
				),
				400 => array(
					'type'    => 'checkbox',
					'label'   => __( 'Darken background to improve readability', 'make-plus' ),
					'name'    => 'darken',
					'default' => ttfmake_get_section_default( 'darken', 'postlist' ),
				),
				500 => array(
					'type'    => 'select',
					'name'    => 'background-style',
					'label'   => __( 'Background style', 'make-plus' ),
					'default' => ttfmake_get_section_default( 'background-style', 'postlist' ),
					'options' => ttfmake_get_section_choices( 'background-style', 'postlist' ),
				),
				600 => array(
					'type'    => 'color',
					...
					'default' => ttfmake_get_section_default( 'background-color', 'postlist' ),
				),
			)
		);
	}
	...
	public function save_section( $data ) {
		...
		$checkboxes = array( 'show-title', 'show-date', 'show-excerpt', 'show-author', 'show-categories', 'show-tags', 'show-comments', 'full-width' );
		...
		// Background style
		$clean_data['background-style'] = ttfmake_sanitize_section_choice( $parsed_data['background-style'], 'background-style', 'postlist' );

		// Background color
		$clean_data['background-color'] = maybe_hash_hex_color( $parsed_data['background-color'] );

		// Columns
		$clean_data['columns'] = ttfmake_sanitize_section_choice( $parsed_data['columns'], 'columns', 'postlist' );

		// Type
		$clean_data['type'] = ttfmake_sanitize_section_choice( $parsed_data['type'], 'type', 'postlist' );

		// Taxonomy
		$clean_data['taxonomy'] = $this->filter()->sanitize_filter_choice( $parsed_data['taxonomy'], $clean_data['type'] );

		// Sort by
		$clean_data['sortby'] = ttfmake_sanitize_section_choice( $parsed_data['sortby'], 'sortby', 'postlist' );
		...
		// Thumbnail
		$clean_data['thumbnail'] = ttfmake_sanitize_section_choice( $parsed_data['thumbnail'], 'thumbnail', 'postlist' );

		// Aspect
		$clean_data['aspect'] = ttfmake_sanitize_section_choice( $parsed_data['aspect'], 'aspect', 'postlist' );
	...
	public function section_defaults( $defaults ) {
		$defaults['postlist'] = $this->get_defaults();
	...
	public function section_choices( $choices, $key, $section_type ) {
		if ( count( $choices ) > 1 || ! in_array( $section_type, array( 'postlist' ) ) ) {
			return $choices;
		}
	...
	public function admin_enqueue( $hook_suffix = '' ) {
		...
		if (
			...
			if ( in_array( $hook_suffix, array( 'post.php', 'post-new.php' ) ) ) {
				wp_enqueue_script(
					'makeplus-postslist-model',
					makeplus_get_plugin_directory_uri() . 'js/postslist/builder-model.js',
					$dependencies,
					MAKEPLUS_VERSION,
					true
				);
				...
				wp_enqueue_script(
					'makeplus-postslist-view',
					makeplus_get_plugin_directory_uri() . 'js/postslist/builder-view.js',
					$dependencies,
					MAKEPLUS_VERSION,
					true
				);
