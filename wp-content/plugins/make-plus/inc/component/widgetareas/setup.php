...
final class MAKEPLUS_Component_WidgetAreas_Setup extends MAKEPLUS_Util_Modules implements MAKEPLUS_Util_HookInterface {
	...
	public function hook() {
		...
		// Full functionality.
			...
			add_action( 'wp_ajax_section_get_widget_data', array( $this, 'ajax_section_get_widget_data' ) );
		}
	...
	public function admin_enqueue( $hook_suffix ) {
		...
		if (
			...
			wp_enqueue_script( 'makeplus-widgetareas-admin' );
	 ...
	 * @return void
	 */
	public function section_text_before_column( $data ) {
		global $ttfmake_is_js_template;
		global $section_name;

		$class = "{{ (get('widget-area') === 1) ? 'active' : 'inactive' }}";
		...
		<input type="hidden" class="ttfmp-text-widget-area" name="<?php echo esc_attr( $section_name ); ?>[widget-area]" value="{{ get('widget-area') }}" />
		<?php
	}
	...
	public function ajax_section_get_widget_data() {
		...
		if ( $sidebar_id ) {
			...
			foreach ( $widget_data as $widget ) {
				$output .= '<li data-id="' . esc_attr( $widget['id'] ) . '">';

				if ( $this->can_edit_widgets() ) {
					$output .= '<div title="' . esc_attr( 'Drag-and-drop this widget into place', 'make-plus' ) . '" class="ttfmake-sortable-handle">';

					$output .= '<div class="sortable-background"></div>';

					$output .= '</div>';
				}

				$output .= '<div class="ttfmp-widget-list-container">';

				$output .= '<span class="ttfmp-widget-list-type">' . wp_strip_all_tags( $widget['type'] ) . '</span>';

				if ( $widget['title'] != '' ) {
					$output .= ': <span class="ttfmp-widget-list-title">' . wp_strip_all_tags( $widget['title'] ) . '</span>';
				}

				if ( $this->can_edit_widgets() ) {
					$output .= '<a href="#" class="edit-widget-link ttfmake-overlay-open" data-overlay="#ttfmake-overlay-' . esc_attr( $widget['id'] ) . '" title="' . esc_attr( 'Configure widget', 'make-plus' ) . '">';

					$output .= '<span>' . esc_html( 'Configure widget', 'make-plus' ) . '</span>';

					$output .= '</a>';

					$output .= '<a href="#" class="remove-widget-link ttfmake-widget-remove" title="' . esc_attr( 'Delete widget', 'make-plus' ) . '">';

					$output .= '<span>' . esc_html( 'Delete widget', 'make-plus' ) . '</span>';
		...
		if ( isset( $widget_data ) && ! empty( $widget_data ) ) {
			foreach ( $widget_data as $widget ) {
				$overlay_output .= $this->overlay_template( $widget['form'], $widget['id_base'], $widget['id'], $widget['type'] );
			}
		}

			$json_output = array(
				'widgets_html' => $output,
				'overlay_html' => $overlay_output
			);
	 ...
	 * @return void
	 */
	public function section_text_after_column( $data ) {
			...
			<div class="ttfmp-widget-area-display">
				<div class="ttfmake-titlediv">
					<?php if ( true === $this->can_edit_widgets() ) : ?>
						<input placeholder="<?php esc_attr_e( 'Enter name here', 'make-plus' ); ?>" type="text" name="ttfmake-section[{{ get('id') }}][columns][{{ get('id') }}][sidebar-label]" class="ttfmake-title" value="{{ get('sidebar-label') }}" autocomplete="off" data-model-attr="sidebar-label" />
					...
					<?php else : ?>
						<input disabled="disabled" type="text" class="ttfmake-title" value="{{ get('sidebar-label') }}" />
					<?php endif; ?>
				</div>

				<div class="ttfmp-widget-area-text">
					<p data-msg-type="no-widgets">
						<?php
					...
					<p data-msg-type="add-more-widgets" style="display: none">
						<?php
							if ( true === $this->can_edit_widgets() ) :
								printf(
								// Translators: %s is a placeholder for a link to the Customizer
					...
					<?php
						if ( $this->can_edit_widgets() ) {
							$ul_class = ' ttfmp-widget-list-sortable';
						}

						echo '<ul class="ttfmp-widget-list' . $ul_class . '"></ul>';
					?>
					</div>
				</div>
			</div>
		...
		if ( 'text' === $section_type ) {
			if ( isset( $original_data['columns'] ) && is_array( $original_data['columns'] ) ) {
				foreach ( $original_data['columns'] as $id => $item ) {
					...
					if ( isset( $item['sidebar-label'] ) && !empty( $item['sidebar-label'] ) ) {
					...
					if ( isset( $item['widget-area'] ) && $item['widget-area'] == 0 ) {
						unset( $clean_data['columns'][ $id ]['widget-area-id'] );
						unset( $clean_data['columns'][ $id ]['widgets'] );
					}

					if( isset( $item['widget-area'] ) && $item['widget-area'] == (int) 1 ) {
						$clean_data['columns'][ $id ]['widget-area-id'] = 'ttfmp-' . get_the_ID() . '-' . $item['parentID'] . '-' . $item['id']; // save widget area ID to match new registered sidebar ID, containing column ID
					}
				}
			}
		}
				...
				if ( isset( $section['columns'] ) ) {
					foreach ( $section['columns'] as $column_id => $column ) {
						if ( isset( $column['widget-area'] ) && 1 === (int) $column['widget-area'] ) {
							...
							if ( $column['widget-area-id'] && !empty( $column['widget-area-id'] ) ) {
								$widget_area_id = $column['widget-area-id'];
							} else {
								$widget_area_id = 'ttfmp-' . get_the_ID() . '-' . $section_id . '-' . $column['id'];
							}
							...
							$new_contents = array(
								...
								'image-link'  => '',
								'image-id'    => '',
								'content'     => '[ttfmp_widget_area page_id="' . absint( get_the_ID() ) . '" section_id="' . ttfmake_get_builder_save()->clean_section_id( $section_id ) . '" widget_area_id="' . $widget_area_id . '"]',
								'widget-area' => 1
							);
							...
							// TODO
							if ( !empty( $widget_area_id ) ) {
								$this->register_sidebar( get_the_ID(), $section_id, $column['id'], $sidebar_label );
							}
	...
	public function widget_area( $attrs ) {
		if( isset( $attrs['widget_area_id'] ) && $attrs['widget_area_id'] ) {
			$id = $attrs['widget_area_id'];
		} else {
			$id = 'ttfmp-' . get_the_ID() . '-' . $attrs['section_id'] . '-' . $attrs['column_id'];
		}
	...
	private function register_sidebar( $page_id, $section_id, $column_id, $label ) {
		$id = absint( $page_id ) . '-' . $this->clean_section_id( $section_id ) . '-' . $column_id;
	...
	public function deleted_post_meta( $meta_ids, $object_id, $meta_key, $_meta_value ) {
		if ( $this->meta_key_is_widget_area( $meta_key ) ) {
			...
			/**
			 * Before 1.8.3, the column IDs were indexes starting from 1 (up to 4).
			 * That's how sidebar ID looked back then, too.
			 * ttfmp-<PAGE_ID>-<SECTION_ID>-<COLUMN_ID>.
			 * Coming from 1.8.2 and older, $column_id above will still be used to
			 * delete sidebar.
			 *
			 * After 1.8.3, column ID is timestamp and is stored in database as `id`.
			 * Sidebar ID now therefore looks different and it's not enough to use
			 * column's index to target sidebar ID.
			 *
			 * `$new_column_id` carries the new ID and we're passing that one to
			 * `deregister_sidebar` if it's found, to keep back compatibility.
			 */
			$new_column_id = get_post_meta( $page_id, '_ttfmake:' . $section_id . ':columns:' . $column_id . ':id', true );

			if ( !empty( $new_column_id ) ) {
