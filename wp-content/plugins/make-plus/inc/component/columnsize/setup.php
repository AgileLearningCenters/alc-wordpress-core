...
final class MAKEPLUS_Component_ColumnSize_Setup implements MAKEPLUS_Util_HookInterface {
	...
	public function admin_enqueue_scripts( $hook_suffix ) {
        ...
        wp_enqueue_script( 'makeplus-columnsize-admin' );
	 ...
	 * @return void
	 */
	public function add_column_inputs( $data ) {
		global $ttfmake_is_js_template;
		global $section_name;
		?>
        <input type="hidden" class="ttfmp-column-size-input" name="<?php echo $section_name; ?>[size]" value="{{ get('size') }}" />
		<?php
	}
	...
	public function add_layout_css( MAKE_Style_ManagerInterface $style ) {
		...
		foreach ( $sections as $id => $data ) {
			if ( isset( $data['section-type'] ) && 'text' === $data['section-type'] && isset( $data['columns-number'] ) ) {
				...
				foreach ( $data['columns'] as $column_number => $column_data ) {
					if ( isset( $column_data['size'] ) && !empty( $column_data['size'] ) ) {
						/*
						 * back compat code for when columns-order is available
						 * which means index starts at 1
						 */
						...
						$style->css()->add( array(
							'selectors'    => array(
								'#' . esc_attr( $section_id ) . $selector_base . $col
							),
							'declarations' => array(
	 ...
	 */
	public function add_classes( $classes, $data ) {
		$classes .= '{{ (get("size")) ? " ttfmake-column-width-"+get("size") : "" }}';
